 ...
 */
export function resolveDataDir({ variable, fallback, env = process.env }) {
  const override = env[variable]
  if (typeof override === 'string' && override.trim() !== '') {
    return { dir: override.trim(), overridden: true, variable }
  }
  return { dir: fallback, overridden: false, variable }
}
